    ...
    <div className="w-full h-full bg-white">
      ...
      {showToast && (
        <div className="fixed top-4 right-4 z-50">
          <div
            className={`flex items-center w-full max-w-xs px-4 py-2 rounded-lg shadow ${
              toastType === "success"
                ? "bg-black text-white md:text-sm text-xs font-semibold"
                : "bg-red-500 text-white md:text-sm text-xs font-semibold"
            }`}
          >
            <div
              className={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-lg ${
                toastType === "success"
                  ? " text-green-500  md:text-base text-sm font-semibold"
                  : " text-red-500  md:text-base text-sm font-semibold"
              }`}
            >
              {toastType === "success" ? "✓" : "⚠️"}
            </div>
